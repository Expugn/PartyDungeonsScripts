/**
 * INTERACT type script for BLOCK (-1489, 169, -998)
 *
 * x4 Drop Rate Shop Interaction
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (item.getType() === Material.PAPER && SpecialItemHandler.compare(player, SpecialItemHandler.lucky_coupon_name.COUPON_x4)) {
        // LUCKY COUPON FOUND
        ...
        dungeon.setTempVariable(var_names.drop_rate, 4);
    ...
    dungeon.messageArea(`${ChatColor.GOLD}${player.getName()} ${ChatColor.GRAY}has purchased the ${ChatColor.GOLD}x4 ${ChatColor.GRAY}drop rate multiplier.`);
    dungeon.setTempVariable(var_names.drop_rate, 4);
}
main();