/**
 * INTERACT type script for BLOCK (-1478, 143, -958)
 *
 * Four Chests One - South West
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.four_chests_one_four)) {
        // CHEST ALREADY TRIGGERED
        player.sendMessage(`${ChatColor.GRAY}The chest is empty!`);
        return;
    }
    dungeon.setTempVariable(var_names.four_chests_one_four, true);

    if (dungeon.getTempVariable(var_names.four_chests_one_value) === 4) {
