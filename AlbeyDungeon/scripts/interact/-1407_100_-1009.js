/**
 * INTERACT type script for BLOCK (-1407, 100, -1009)
 *
 * Emerald Room Four - North West
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.emerald_four_clear)) {
        // DOOR IS ALREADY OPEN
        player.sendMessage(`${ChatColor.GRAY}The door is already open.`);
        return;
    }

    if (dungeon.getTempVariable(var_names.emerald_four_three)) {
        // BLOCK ALREADY INTERACTED WITH
        player.sendMessage(`${ChatColor.GRAY}This block has already been interacted with.`);
        return;
    }
    dungeon.setTempVariable(var_names.emerald_four_three, true);

    if (dungeon.getTempVariable(var_names.emerald_four_value) === 3) {
        ...
        dungeon.setTempVariable(var_names.emerald_four_clear, true);
        ScheduleHandler(() => {
            doors.emerald_four_east.run_east(true);
        });
        return;
    }
