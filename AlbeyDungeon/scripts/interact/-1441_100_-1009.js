/**
 * INTERACT type script for BLOCK (-1441, 100, -1009)
 *
 * Emerald Room Three - North West
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.emerald_three_three)) {
        // BLOCK ALREADY INTERACTED WITH
        player.sendMessage(`${ChatColor.GRAY}This block has already been interacted with.`);
        return;
    }
    dungeon.setTempVariable(var_names.emerald_three_three, true);

    if (dungeon.getTempVariable(var_names.emerald_three_value) === 3) {
