/**
 * INTERACT type script for BLOCK (-1457, 132, -1038)
 *
 * Four Chests Two - South East
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.four_chests_two_three)) {
        // CHEST ALREADY TRIGGERED
        player.sendMessage(`${ChatColor.GRAY}The chest is empty!`);
        return;
    }
    dungeon.setTempVariable(var_names.four_chests_two_three, true);

    if (dungeon.getTempVariable(var_names.four_chests_two_value) === 3) {
        // THIS CHEST HAS THE KEY
        dungeon.messageParty(`${ChatColor.GOLD}${player.getName()} ${ChatColor.GRAY}obtained a ${ChatColor.GOLD}Key ${ChatColor.GRAY}from a chest.`);
