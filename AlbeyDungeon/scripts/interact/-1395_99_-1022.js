/**
 * INTERACT type script for BLOCK (-1395, 99, -1022)
 *
 * Four Chests Three - South East
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.four_chests_three_three)) {
        // CHEST ALREADY TRIGGERED
        player.sendMessage(`${ChatColor.GRAY}The chest is empty!`);
        return;
    }
    dungeon.setTempVariable(var_names.four_chests_three_three, true);

    if (dungeon.getTempVariable(var_names.four_chests_three_value) === 3) {
