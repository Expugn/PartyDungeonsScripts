/**
 * INTERACT type script for BLOCK (-1457, 132, -1042)
 *
 * Four Chests Two - North East
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.four_chests_two_four)) {
        // CHEST ALREADY TRIGGERED
        player.sendMessage(`${ChatColor.GRAY}The chest is empty!`);
        return;
    }
    dungeon.setTempVariable(var_names.four_chests_two_four, true);

    if (dungeon.getTempVariable(var_names.four_chests_two_value) === 4) {
