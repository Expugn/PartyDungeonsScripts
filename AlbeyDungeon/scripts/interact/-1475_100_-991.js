/**
 * INTERACT type script for BLOCK (-1475, 100, -991)
 *
 * Emerald Room Two - South West
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.emerald_two_four)) {
        // BLOCK ALREADY INTERACTED WITH
        player.sendMessage(`${ChatColor.GRAY}This block has already been interacted with.`);
        return;
    }
    dungeon.setTempVariable(var_names.emerald_two_four, true);

    if (dungeon.getTempVariable(var_names.emerald_two_value) === 4) {
