/**
 * INTERACT type script for BLOCK (-1475, 133, -959)
 *
 * Emerald Room One - North East
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.emerald_one_three)) {
        // BLOCK ALREADY INTERACTED WITH
        player.sendMessage(`${ChatColor.GRAY}This block has already been interacted with.`);
        return;
    }
    dungeon.setTempVariable(var_names.emerald_one_three, true);

    if (dungeon.getTempVariable(var_names.emerald_one_value) === 3) {
