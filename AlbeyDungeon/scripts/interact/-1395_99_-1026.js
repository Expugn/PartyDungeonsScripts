/**
 * INTERACT type script for BLOCK (-1395, 99, -1026)
 *
 * Four Chests Three - North East
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.four_chests_three_four)) {
        // CHEST ALREADY TRIGGERED
        player.sendMessage(`${ChatColor.GRAY}The chest is empty!`);
        return;
    }
    dungeon.setTempVariable(var_names.four_chests_three_four, true);

    if (dungeon.getTempVariable(var_names.four_chests_three_value) === 4) {
