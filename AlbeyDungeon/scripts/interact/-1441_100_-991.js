/**
 * INTERACT type script for BLOCK (-1441, 100, -991)
 *
 * Emerald Room Three - South West
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.emerald_three_four)) {
        // BLOCK ALREADY INTERACTED WITH
        player.sendMessage(`${ChatColor.GRAY}This block has already been interacted with.`);
        return;
    }
    dungeon.setTempVariable(var_names.emerald_three_four, true);

    if (dungeon.getTempVariable(var_names.emerald_three_value) === 4) {
