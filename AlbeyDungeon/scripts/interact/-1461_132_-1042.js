/**
 * INTERACT type script for BLOCK (-1461, 132, -1042)
 *
 * Four Chests Two - North West
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.four_chests_two_two)) {
        // CHEST ALREADY TRIGGERED
        player.sendMessage(`${ChatColor.GRAY}The chest is empty!`);
        return;
    }
    dungeon.setTempVariable(var_names.four_chests_two_two, true);

    if (dungeon.getTempVariable(var_names.four_chests_two_value) === 2) {
