/**
 * INTERACT type script for BLOCK (-1389, 100, -1009)
 *
 * Emerald Room Four - North East
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.emerald_four_one)) {
        // BLOCK ALREADY INTERACTED WITH
        player.sendMessage(`${ChatColor.GRAY}This block has already been interacted with.`);
        return;
    }
    dungeon.setTempVariable(var_names.emerald_four_one, true);

    if (dungeon.getTempVariable(var_names.emerald_four_value) === 1) {
