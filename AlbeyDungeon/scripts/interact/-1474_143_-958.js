/**
 * INTERACT type script for BLOCK (-1474, 143, -958)
 *
 * Four Chests One - South East
 *
 * @author      Expugn
 * @version     0.1
 * @type        INTERACT
...
function main() {
    ...
    if (dungeon.getTempVariable(var_names.four_chests_one_two)) {
        // CHEST ALREADY TRIGGERED
        player.sendMessage(`${ChatColor.GRAY}The chest is empty!`);
        return;
    }
    dungeon.setTempVariable(var_names.four_chests_one_two, true);

    if (dungeon.getTempVariable(var_names.four_chests_one_value) === 2) {
